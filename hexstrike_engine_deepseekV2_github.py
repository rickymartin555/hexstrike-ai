...
Changelog v14.1.0-r3:
  ...
  FIX #4  – run_scan returns (findings, folder, chain_engine, tech_stack) 4-tuple; main() unpacks
  ...
  NEW     – --bug-bounty-user CLI arg (or BUG_BOUNTY_USER env var); auto sha256 for non-alphanumeric
  NEW     – BUG_BOUNTY_HEADER global injected into HTTP session, per-request, MCP, OOB poll, LLM calls
  NEW     – --extra-header / --rate-limit / --user-agent CLI args
  NEW     – EXTRA_HEADERS global merged into every HTTP request
  NEW     – Config.RATE_LIMIT field used by TokenBucket
"""

__version__ = "14.1.0-r3"
...
# ---------------------------------------------------------------------------
# Bug-bounty header helper  (PATCH 2)
# ---------------------------------------------------------------------------
def compute_bug_bounty_header_value(username: str) -> str:
    """
    Returns username as-is if purely alphanumeric (a-z, A-Z, 0-9).
    Returns sha256(username) hex digest if username contains any non-alphanumeric
    character.  Per Intigriti bug bounty programme rules.
    """
    if not username:
        return ""
...
# NEW – populated in main() from --extra-header
EXTRA_HEADERS: Dict[str, str] = {}

# Populated in main() from --bug-bounty-user / BUG_BOUNTY_USER env var (PATCH 2)
# (redeclared here so the name is available before main(); main() overwrites it)
...
class StreamingHTTPClient:
    ...
    def _create_session(self):
        ...
        if BUG_BOUNTY_HEADER:
            session.headers.update(BUG_BOUNTY_HEADER)
        return session
    ...
    def request(
        ...
        # Merge global extra headers (NEW – --extra-header)
        req_headers.update(EXTRA_HEADERS)
        # Merge bug-bounty header (PATCH 3 – --bug-bounty-user)
        if BUG_BOUNTY_HEADER:
    ...
    def check_callback(self, token: str, timeout: int = 30) -> bool:
        ...
        while time.time() < deadline:
            try:
                ...
                resp = _requests.get(poll_url, headers=poll_headers, timeout=5)
...
# Main scan pipeline
# ---------------------------------------------------------------------------
def run_scan(target: str) -> Tuple[List[Dict], str, "AttackChainEngine", Dict]:
    """
    FIX #4 – return (findings, folder, chain_engine, tech_stack) as a 4-tuple.
    ...
    # FIX #4 – return all four items so main() can pass them to generate_report
...
# ---------------------------------------------------------------------------
def main() -> None:
    global EXTRA_HEADERS, BUG_BOUNTY_HEADER
...
""",
    ...
    # Bug-bounty / scope-compliance CLI arguments (PATCH 2)
    parser.add_argument(
        "--bug-bounty-user",
        default=os.environ.get("BUG_BOUNTY_USER", ""),
        help=(
            "Your Intigriti/bug-bounty username for the X-Bug-Bounty request header. "
            "Alphanumeric usernames are sent as-is; others are sha256-hashed automatically. "
            "Can also be set via the BUG_BOUNTY_USER environment variable."
        ),
    )
    parser.add_argument(
        "--extra-header",
    ...
    # ── Populate BUG_BOUNTY_HEADER global (PATCH 2) ───────────────────────
    if args.bug_bounty_user:
        _hv = compute_bug_bounty_header_value(args.bug_bounty_user)
        BUG_BOUNTY_HEADER = {"X-Bug-Bounty": _hv}
        console.print(f"[green][BugBounty] X-Bug-Bounty: {_hv}[/green]")
    else:
        console.print("[yellow][BugBounty] --bug-bounty-user not set; X-Bug-Bounty will NOT be sent.[/yellow]")
    ...
    # ── FIX #4 / PATCH 5: unpack 4-tuple from run_scan ───────────────────
    findings, folder, chain_engine, tech_stack = run_scan(args.target)
